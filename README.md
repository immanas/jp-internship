# 💼 JPMorgan Chase & Co. – Software Engineering Virtual Experience

![Forage](https://img.shields.io/badge/Forage-JPMorgan%20Chase-blue?style=flat-square)
![Java](https://img.shields.io/badge/Java-Spring%20Boot-orange?style=flat-square)
![Kafka](https://img.shields.io/badge/Apache-Kafka-black?style=flat-square)
![Status](https://img.shields.io/badge/Status-Completed-brightgreen?style=flat-square)

> Virtual Experience Program by JPMorgan Chase & Co. via Forage  
> **Completed:** November 2025 · [View Certificate](https://www.linkedin.com/posts/me-m-gantait_jpmorgan-activity-7399905226922782720-Pnpb?utm_source=share&utm_medium=member_desktop&rcm=ACoAAEZICGgBQrTuMWLb1ODABZ-c0r-oONAbnuM)


## 📌 About

This repository documents my completion of the **JPMorgan Chase Software Engineering Virtual Experience** on Forage. The program simulates backend engineering tasks used in real-world financial systems — covering event-driven architecture, REST API design, and data persistence.

The work here reflects hands-on exposure to the kind of backend infrastructure that powers large-scale financial platforms.

## 🧩 Tasks Completed

### Task 1 — Environment & Project Setup
- Configured a production-like backend environment using Spring Boot
- Structured codebase following separation of concerns (controllers, services, config)

### Task 2 — Apache Kafka Integration
- Implemented event-driven data streaming using Apache Kafka
- Simulated real-time financial transaction flow between producer and consumer services

### Task 3 — H2 In-Memory Database
- Integrated H2 database for lightweight data persistence
- Wrote schema definitions and handled CRUD operations via JPA

### Task 4 — REST API Integration
- Consumed and exposed RESTful endpoints between services
- Handled request/response lifecycle with proper status codes and error handling

### Task 5 — REST API Controller Development
- Built Spring Boot controllers to manage routing and business logic delegation
- Followed MVC pattern for clean separation of concerns

---

## 🛠 Tech Stack

| Layer | Technology |
|---|---|
| Language | Java |
| Framework | Spring Boot |
| Messaging | Apache Kafka |
| Database | H2 (In-Memory) |
| API Style | REST |

---

## 📁 Project Structure
```
jp-virtual-experience/
│
├── src/
│   ├── controllers/       # REST API controllers
│   ├── services/          # Business logic layer
│   ├── models/            # Data models / entities
│   └── config/            # Kafka & app configuration
│
├── database/              # H2 schema and setup
├── resources/             # application.properties
└── README.md
```

---

## 💡 Key Takeaways

- Understood **event-driven architecture** and how Kafka decouples services at scale
- Practiced **REST API design principles** used in microservices environments
- Gained exposure to **backend system design patterns** in financial systems
- Recognized parallels to **DevOps concerns** — service communication, config management, and pipeline-ready code structure

## 📜 Certificate

Verified completion via Forage:  
🔗 [JPMorgan Chase Software Engineering Virtual Experience – Forage](YOUR_CERTIFICATE_URL)

---

## ⚠️ Disclaimer

This is a **virtual experience program** completed via [Forage](https://www.theforage.com/), 
not a full-time or part-time internship. It is intended to simulate real engineering 
tasks and build industry-relevant skills.

*Completed as part of active upskilling toward DevOps & Cloud Engineering roles.*
*Completed via Forage – JPMorgan Chase & Co. Virtual Experience Program*
